...
/// Generated class from Pigeon that represents data sent in messages.
struct WarmAlarmRemediationResultWire: Hashable {
  ...
  static func == (lhs: WarmAlarmRemediationResultWire, rhs: WarmAlarmRemediationResultWire) -> Bool {
    ...
    return deepEqualsMessages(lhs.status, rhs.status) && deepEqualsMessages(lhs.permissionState, rhs.permissionState) && deepEqualsMessages(lhs.readiness, rhs.readiness)
  }

  func hash(into hasher: inout Hasher) {
    hasher.combine("WarmAlarmRemediationResultWire")
    deepHashMessages(value: status, hasher: &hasher)
    deepHashMessages(value: permissionState, hasher: &hasher)
