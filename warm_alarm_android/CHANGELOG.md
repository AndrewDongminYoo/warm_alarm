...
- End any wake-check cycle still in flight when an alarm is replaced. The retrigger is a separate `PendingIntent` now, so re-scheduling no longer retargets it and it would otherwise ring at the old delay.
