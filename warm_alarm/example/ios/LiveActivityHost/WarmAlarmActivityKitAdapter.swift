...
private final class WarmAlarmActivityKitAdapter: WarmAlarmLiveActivityAdapter {
    ...
    func update(
        ...
        let reply = WarmAlarmLiveActivityReply(completion)
        Task { @MainActor in
            guard let activity = Self.mutableActivity(withId: activityId) else {
                reply(.success(false))
                return
            }
    ...
    private static func mutableActivity(
        withId activityId: String
    ) -> Activity<WarmAlarmActivityAttributes>? {
        Activity<WarmAlarmActivityAttributes>.activities.first { activity in
            activity.id == activityId && (activity.activityState == .active || activity.activityState == .stale)
        }
    }
